...
<script>
    function toggleFilterSarana() {
        const content = document.getElementById('filterContentSarana');
...
<div class="page-wrap">
    ...
    <div class="relative rounded-2xl bg-gradient-to-r from-slate-800 to-blue-900 p-6 shadow-lg mb-8 text-white flex flex-col md:flex-row items-center justify-between gap-4 border border-blue-800/50 overflow-hidden">
        ...
        <div class="relative z-10 w-full md:w-auto">
            ...
            <p class="text-blue-100 text-xs md:text-sm opacity-80 mt-1">Dashboard monitoring surat panggilan dan kasus siswa terkait sarana prasarana.</p>
        </div>
        ...
        <div class="hidden xs:flex items-center gap-3 bg-white/10 backdrop-blur-md px-4 py-3 rounded-2xl border border-white/10 shadow-inner min-w-[140px] relative z-10">
            <div class="bg-blue-500/20 p-2 rounded-lg text-blue-200">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/></svg>
            </div>
            <div>
    ...
    {{-- Filter & Pencarian Section (Mirip Kesiswaan) --}}
    <div id="saranaFilterCard" class="bg-white rounded-2xl shadow-soft border border-slate-200 overflow-hidden mb-6">
        <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center cursor-pointer transition-colors hover:bg-slate-50 group" onclick="toggleFilterSarana()">
        ...
        <div id="filterContentSarana" class="p-6 transition-all duration-300">
            <form action="{{ route('dashboard.waka-sarana') }}" method="GET" class="w-full">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                    <div class="col-span-1">
                        ...
                        <input type="date" name="start_date" value="{{ $startDate }}" class="w-full bg-slate-50 border border-slate-200 text-slate-700 text-sm rounded-xl block p-2.5 outline-none">
                    </div>
                    <div class="col-span-1">
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 ml-1">Sampai Tanggal</label>
                        <input type="date" name="end_date" value="{{ $endDate }}" class="w-full bg-slate-50 border border-slate-200 text-slate-700 text-sm rounded-xl block p-2.5 outline-none">
                    </div>
                    <div class="col-span-1 lg:col-span-2"></div> {{-- Spacer --}}
                    <div class="col-span-1 flex gap-2">
                        <button type="submit" class="flex-1 inline-flex justify-center items-center gap-2 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold uppercase rounded-xl shadow-md transition-all h-[42px] border-none cursor-pointer">
                            Filter
                        </button>
                    </div>
                </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div class="stat-card hover-lift border-l-4 border-l-emerald-500">
        ...
        <div class="stat-card hover-lift border-l-4 border-l-rose-500">
            <div class="flex justify-between items-start mb-3">
        ...
        <div class="stat-card hover-lift border-l-4 border-l-amber-500">
            <div class="flex justify-between items-start mb-3">
        ...
        <div class="stat-card hover-lift border-l-4 border-l-blue-500">
            <div class="flex justify-between items-start mb-3">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left table-fixed">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        ...
                        @empty
                        <tr><td colspan="3" class="text-center py-12 text-slate-400 text-xs italic">Tidak ada kasus surat yang melibatkan Waka Sarana.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
...
<script>
// Konfigurasi Horizontal Bar (Identik dengan Kesiswaan)
